...
select dp.category,
       ...
       DENSE_RANK()over(partition by dp.category order by sum(fs.sales_amount)desc) as rn
       ...
       (select category,
               ...
               (select category,
                       ...
                       select *, 
                              round(100.0*top5sales/categorysales,2) as pctsales
                              from cte2 as c2
                              join cte3 as c3
                              on c2.category = c3.category
                              where c2.top5sales > 0.40*c3.categorysales
